...
>
> ⚠️ **注意**：本仓库包含脱敏后的证据样例、数据契约 (Schema) 及自检脚本。
> **核心算法与训练管线 (HoloForge-Core) 未在此开源**，本项目仅用于展示验证结果与工程化质量门禁。
...
*   **技术路径**：采用**基于 Hessian 敏感度的自适应分层 (Adaptive Layering)**，智能识别关键层并保留高精度，仅对冗余层进行激进压缩。
...
*   **技术路径**：实现 **Windowed-loading (开窗加载)** 动态调度机制，将算子级内存峰值增量严格控制在 **444MiB**。
